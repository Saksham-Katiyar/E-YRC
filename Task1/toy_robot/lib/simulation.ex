defmodule ToyRobot.CLI do
  def main(_args) do
    IO.puts("Welcome to the Toy Robot Simulation!!")
    print_help_messages()
    receive_command()
  end
  ...
  @commands %{
    ...
    "place" => "format: \"place [X,Y,F]\". " <>
              ...
              "Where facing is: north, west, south, east.",
    "report" => "The Toy Robot reports about its position and facing direction",
  ...
  defp print_help_messages do
    ...
    @commands |> Enum.map(fn({command, description}) -> IO.puts("  #{command} - #{description}") end)
  end
