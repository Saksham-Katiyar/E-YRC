defmodule ToyRobot do
  ...
  def traverse(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, obs_map, goal_x, goal_y, cli_proc_name) when x==goal_x and y==goal_y do
    is_obstacle=send_robot_status(robot, cli_proc_name)
    {:ok, robot}
  end
  def traverse(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, obs_map, goal_x, goal_y, cli_proc_name) do
    ...
    index=0
    shortest_path=make_shortest_path(shortest_path,goal_index,index,obs_map)
  ...
  def make_shortest_path(shortest_path,goal_index,index,obs_map) when Enum.at(shortest_path, index)==goal_index do
    li=[]
    find_shortest_path(shortest_path,index,li)
  end
  def make_shortest_path(shortest_path,goal_index,index,obs_map) do
    shortest_path=shortest_path++find_neighbour_indexes(Enum.at(shortest_path,index),obs_map)
    make_shortest_path(shortest_path,goal_index,index+1,obs_map)
  end
  def find_neighbour_indexes(position_index,obs_map) do
    ...
    neighbour_indexes=if position_index==-1 do
                        ...
                        neighbour_indexes=[]
                        neighbour_indexes=if position_index<20 and obs_map((@robot_map_y_atom_to_num[y])*5+x+14)!=1 do
                                            List.insert_at(neighbour_dists, 0, position_index+5))
                                          else
                                            List.insert_at(neighbour_dists, 0, -1)
                                          end
                        neighbour_indexes=if rem(position_index+1,5)!=0 and obs_map((@robot_map_y_atom_to_num[y])*4+x-5)!=1 do
                                            List.insert_at(neighbour_dists, 1, position_index+1)
                                          else
                                            List.insert_at(neighbour_dists, 1, -1)
                                          end
                        neighbour_indexes=if position_index>4 and obs_map((@robot_map_y_atom_to_num[y])*5+x+9)!=1 do
                                            List.insert_at(neighbour_dists, 2, position_index-5)
                                          else
                                            List.insert_at(neighbour_dists, 2, -1)
                                          end
                        neighbour_indexes=if rem(position_index,5)!=0 and obs_map((@robot_map_y_atom_to_num[y])*4+x-6)!=1 do
                                            List.insert_at(neighbour_dists, 3, position_index-1)
                                          else
  ...
  def move_on_shortest_path(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, shortest_path, index, cli_proc_name, obs_map) when index==length(shortest_path)-1 do
    robot
  end
  def move_on_shortest_path(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, shortest_path, index, cli_proc_name, obs_map) do
    is_obstacle=send_robot_status(robot,cli_proc_name)
  ...
  def move_to_next_index(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, index_0, index_1, cli_proc_name,obs_map) do
    direction=cond do
    ...
    robot=cond do
      ...
      @directions_to_the_right[facing]==direction -> right(robot)
      @directions_to_the_left[facing]==direction -> left(robot)
      true -> right(right(robot))
    end
    is_obstacle=send_robot_status(robot,cli_proc_name)
    obs_map=update_obs_map(robot, obs_map, is_obstacle)
    robot=if is_obstacle==false do
  ...
  def update_obs_map(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, obs_map, is_obstacle) do
    obs_map= if is_obstacle==true do
      obs_map=if facing==:east do
                index=(@robot_map_y_atom_to_num[y])*4+x-5
                obs_map=List.delete_at(obs_map,index)
