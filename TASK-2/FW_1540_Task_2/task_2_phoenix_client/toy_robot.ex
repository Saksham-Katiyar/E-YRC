defmodule ToyRobot do
  ...
  """
  def stop(%ToyRobot.Position{x: _x, y: _y, facing: _facing} = robot, goal_x, goal_y, channel) do
    Process.register(self(), :client_toyrobot)
    traverse(robot, goal_x, goal_y, channel)
  end

  defp traverse(%ToyRobot.Position{x: x, y: y, facing: _facing} = robot, goal_x, goal_y, channel) when x == goal_x and y == goal_y do
    _is_obstacle = ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot)
    {:ok, robot}
  end

  defp traverse(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, goal_x, goal_y, channel) do
    is_obstacle_0 = ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot)
  ...
  defp left_until_free(robot, channel) do
    robot = left(robot)
    if ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot) or !move_possible(robot) do
  ...
  defp obstacle_sequence(robot, channel) do
    ...
    robot = move(robot)
    _ = ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot)
    robot = right(robot)
    if !ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot) and move_possible(robot) do
      move(robot)
    else
      obstacle_sequence(robot, channel)
    end
